#ifndef __INFLATE_PLATFORM_HPP
#define __INFLATE_PLATFORM_HPP
...
///
/// * `INFLATE_SHARED`: defined when compiling or importing as a shared object
/// * `INFLATE_EXPORT`: defined when compiling as a shared object, explicitly noting that symbols should be exported,
///                       not imported.
///
/// ## Macros defined by the compiler state
///
/// * `INFLATE_WIN32`: defined when compiling on MSVC
/// * `EXPORT`: on MSVC, if `INFLATE_SHARED` is defined and `INFLATE_EXPORT` is defined, this value is
///             `__declspec(dllexport)`; if `INFLATE_EXPORT` is not defined, this is defined as
...
#define INFLATE_WIN32
#endif

#if defined(INFLATE_SHARED)

#if defined(INFLATE_WIN32)

#if defined(INFLATE_EXPORT)
